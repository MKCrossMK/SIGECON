...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('policies', function (Blueprint $table) {
            ...
            $table->float('c_interest_pay' , 12, 2); // interes a pagar expresado en pesos
            $table->float('base_interest_pay', 12, 2); //base de interes en numeros
            $table->float('contract_pay' , 12, 2); // monto de contraro a pagar 
            $table->float('c_contract_pay', 12, 2); // monto de contraro a pagar 
            $table->float('base_contract_pay', 12, 2);  // base de contrato a pagar
            ...
            $table->foreign('referrer_id')->references('id')->on('policy_referrers');
