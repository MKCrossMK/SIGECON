...
function update_total(){
    ...
    $(".loan_price").each(function(i){
      ...
      if (price == 0 || price == "") {
        $(this).focus();
        error_notify("Precio en 0 o Vacío", "Ha olvidado colocar valor en articulo, coloque valor correspondiente del articulo")
...
$(document).ready(function(){
 ...
 let id_img = 0;
    ...
    //   let imgInp  = document.getElementById("a_image");
      ...
      if (description != "" && carat != ""  && weight != "" && valued_price != "" && loan_price != "" && loan_price > 0 && imagen.files.length > 0) {

    $('.item-row:last').after('<div class="item-row"><div class="div_article mb-2 ">'
    + '<div class="row align-items-center">'
    +  '<div class="col-1">'
    +       '<button type="button" class="btn btn_delete rounded-circle delete"><i class="fa-solid fa-trash-can"></i></button>'
    +  '</div>'
    +   '<div class="col-3">'
    +        '<img class="img-fluid img_article" id="prev_img'+ id_img +'" alt="Imagen Articulo">'
    +        '<input type="file" name="image[]" id="image'+id_img+'" hidden>'
    +   '</div>'
    +   '<div class="col">'
    +        '<textarea id="description" name="description[]" class="form-control w-100 mb-2" rows="1">' + description+ '</textarea>'
    +       '<div class="row mb-2">'
    +           '<div class="col">'
    +               '<div class="input-group">'
    +                  '<span class="input-group-text" id="basic-addon1">Quilate (K)</span>'
    +                  '<input type="text" id="carat" name="carat[]" class="form-control" value="'+ carat +'" readonly>'
    +               '</div>'
    +            '</div>'
    +            '<div class="col">'
    +                '<div class="input-group">'
    +                    '<span class="input-group-text" id="basic-addon1">T. Piedra</span>'
    +                    '<input type="text" id="stone_type" name="stone_type[]" class="form-control" value="'+ stone_type +'" readonly>'
    +                '</div>'
    +            '</div>'
    +        '</div>'
    +        '<div class="row mb-2">'
    ...
    +                    '<input type="text" id="valued_price" name="valued_price[]" class="form-control" value="'+ valued_price +'" readonly>'
    +                '</div>'
    +            '</div>'
    +        '</div>'
    +        '<div class="input-group">'
    +            '<span class="input-group-text" id="basic-addon1">P. Préstamo</span>'
    +            '<input type="text" id="loan_price" name="loan_price[]" class="form-control loan_price" value="'+ loan_price +'" onblur="readonly_price(this)"  readonly>'
    +        '</div>'
    +    '</div>'
    + '</div>'
  + '</div>'
    ...
    if($("#client_id").val() == ''){
        ...
        $('#send_pay').prop('disabled', false);
            ...
            new Notify({
                ...
                text: "Rellene todos los campos requeridos del articulo, estos son: || Descripción del articulo || Imagen || Quilates || Peso del articulo ||",
                effect: "slide",
                speed: 300,
                showIcon: true,
                showCloseButton: false,
...
$(document.body).on('click', '.loan_price' ,function(){
  ...
  function check_values(){
    ...
    if (pass_gerence_to_preference_rate == false && pass_gerence_to_second_rate == false) {
        ...
        } else if(pass_gerence_to_preference_rate == true && pass_gerence_to_second_rate == false){
            $("#interest_rate").val(interest_preference);
            update_total();
        }
        else if(pass_gerence_to_second_rate == true && pass_gerence_to_preference_rate == false){
            $("#interest_rate").val(interest_second);
            update_total();
        } else if(pass_gerence_to_preference_rate == true && pass_gerence_to_second_rate == true){
...
// Esta funcion verifica el valor de la prenda 
function check_exist_articles(){
  ...
  $("#interest_preference").click(function () {
    if(pass_gerence_to_preference_rate == false ){
        Swal.fire({
            title: `Tasa Preferencial de ${$(this).val()}`,
            ...
            preConfirm: (code) => {
                ...
                (async () => {
                    const rawResponse = await fetch(
                        window.location.origin + "/api/policies/checkpassword",
                        {
                            method: "POST",
                            headers: {
                                Accept: "application/json",
                    ...
                    if(data === true){
                        pass_gerence_to_preference_rate = data;
                        check_values();
                        success_notify('Excelente', `Tasa preferencial de ${$(this).val()}% fue asignada`)
                    }else{
                        check_values();
                        error_notify('Error', 'Gerente, confirme que el codigo sea el correcto; Si el error persiste comunicarse con dpto. de tecnología')
...
$("#interest_second").click(function () {
    if(pass_gerence_to_second_rate == false){
        Swal.fire({
            title: `Tasa de ${$(this).val()} %` ,
            text: "Para colocar esta tasa, el gerente de este sucursal debe autorizarlo mediante su contraseña de usuario",
            input: "password",
            inputAttributes: {
                autocapitalize: "off",
                autocorrect: 'off'
            ...
            preConfirm: (code) => {
                ...
                (async () => {
                    ...
                    if(data === true){
                        pass_gerence_to_second_rate = data;
                        check_values();
                        success_notify('Excelente', `Tasa de ${$(this).val()}% fue asignada`)
...
function showTablePolicyPay () {

}

// Abonos a poliza view

function skipInterestRateMonth (idPolicy) {
    Swal.fire({
        title: `Omitir Aumento de Tasa Interes del Mes` ,
        text: "Acto de gracia entre los 3 primeros dias, para clientes que reclaman, para exceptuar aumento mensual del interes de este mes, el gerente debe colocar su codigo unico",
        input: "password",
        inputAttributes: {
            autocapitalize: "off",
            autocorrect: 'off'
        ...
        preConfirm: (code) => {
            ...
            (async () => {
                ...
                if(data === true){
                    fetch(window.location.origin + `/api/policies/interest_rate_month/skip/${idPolicy}`)
                    .then((response) => response.json()).then((data) => {
                        if(data == true){
                            success_notify('Listo', 'Se ha omitido el interes de este mes, se actualizará la pagina en 5 segundos');
                            ...
                            error_notify('Error', 'Error no se puede omitir el interes de la poliza');
                        }
                    });

                }else{
                    error_notify('Error', 'Gerente, confirme que el codigo sea el correcto; Si continua el error, favor comunicarse con dpto. de tecnología');
                }
