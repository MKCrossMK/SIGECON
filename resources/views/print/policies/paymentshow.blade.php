<x-app-layout>
    ...
    <div id="border_container" class="page-content container">
        <div class="page-header text-blue-d2">
            <a  id="btn_back_btn"  href="{{ route('policies.show',$policyPayment->policy->id  ) }}" class="btn btn_blue" title="Volver"><i
                    class="fa-sharp fa-solid fa-circle-arrow-left"></i></a>

            <h1 id="num_policy" class="page-title text-secondary-d1">
                <small class="page-info">
                    <i class="fa fa-angle-double-right text-80"></i>
                    Nº Poliza : {{ $policyPayment->policy->number_policy }}
                </small>
            </h1>
        ...
        <div>
            ...
            <div class="cabecera-head text-center">
                ...
                <h5>RNC: 401007632 </h5>
                <strong class="uppercase">Surcursal {{ $policyPayment->policy->branch_office->name }}</strong>
        ...
        <div>
            ...
            <h3 class="text-right  font-bold">Fecha: <span class="ml-3">{{date('d-m-Y', strtotime($policyPayment->date_paid))}}</span></h3>   
            <h3 class="text-right">{{$policyPayment->policy->branch_office->province}}</h3>             
        </div>

        <div class="mt-5">
            <h2 class="font-bold uppercase">Nombre: <span class="font-normal uppercase">{{$policyPayment->policy->clients->name . " " . $policyPayment->policy->clients->lastname}}</span></h2>
            <h3 class="font-bold uppercase">Cedula: <span class="font-normal">{{$policyPayment->policy->clients->cedula}}</span></h3>
        </div>

        <div class="mt-5">
            <h2 class="font-bold">Poliza No: <span class="ml-3">&nbsp; {{$policyPayment->policy->number_policy}}</span></h2>
        </div>


        <div class="mt-5 mb-5">
            <h2 class="font-bold">Fecha de Otorgamiento: <span class="font-normal ml-3">&nbsp; {{$policyPayment->policy->date_start}}</span></h2>
            <h3 class="font-bold ">Fecha de Vencimiento: <span class="font-normal ml-3">&nbsp;&nbsp; &nbsp;&nbsp;{{$policyPayment->policy->date_end}}</span></h3>
        </div>
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <td class="text-center" scope="col">DOP {{number_format($policyPayment->amount, 2)}}</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <th class="text-center">Balance capital:</td>
                            <td class="text-center">DOP {{ number_format($policyPayment->policy->capital_pay, 2)}}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
